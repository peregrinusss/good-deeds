...
headerMenu();
const accordions = document.querySelectorAll(".accordion"); //находим все аккордионы
...
const selects = document.querySelectorAll(".help-select");
if (selects) {
  selects.forEach(select => {
    const control = select.querySelector(".help-select__title"); //
    const content = select.querySelector(".help-select__list"); //то что будем открывать
...
function filterFunds() {
  const buttons = document.querySelectorAll('.partner__filter');
  const cards = document.querySelectorAll('.partner__card');
...
if (swiperFoucs) {
  const workSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](swiperFoucs, {
    autoplay: 2500,
    speed: 800,
    effect: 'fade',
    fadeEffect: {
...
const swiperEvent = document.querySelector(".swiper--event");
if (swiperEvent) {
...
const swiperOther = document.querySelector(".swiper-other");
if (swiperOther) {
...
const swiperCollect = document.querySelector('.col-slider');
if (swiperCollect) {
  const sport_boley__block_swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](swiperCollect, {
    loop: true,
    keyboard: {
      enabled: true
    },
...
const swiperSpecial = document.querySelector(".special-slider");
if (swiperSpecial) {
  const workSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](swiperSpecial, {
    slidesPerView: 1.1,
    slidesPerGroup: 1,
    spaceBetween: 24,
    loop: true,
...
const swiperRequest = document.querySelector(".swiper-request");
if (swiperRequest) {
  const workSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](swiperRequest, {
    slidesPerView: 1.2,
    slidesPerGroup: 1,
    spaceBetween: 20,
    // Navigation arrows
    navigation: {
      nextEl: ".request__next",
      prevEl: ".request__prev"
    },
    pagination: {
...
$("#fund-tel").inputmask('+7(999)999-99-99');

// Cropp file ava
const cropAvaPopup = document.querySelector('.popup-cropp');
if (cropAvaPopup) {
  ...
  const croppCancelBtn = document.querySelector('.cropp-cancel');
  let croppieInstance;
  ...
  inputPhoto.addEventListener('change', event => {
    ...
    cropAvaPopup.classList.add('opened');
    reader.onload = () => {
  ...
  croppCancelBtn.addEventListener('click', () => {
    cropAvaPopup.classList.remove('opened');
    moderatePopup.classList.add('graph-modal-open', 'fade', 'animate-open');
    inputPhoto.value = ''
  });
  ...
  cropBtn.addEventListener('click', () => {
    cropAvaPopup.classList.remove('opened');
    // moderatePopup.classList.add('graph-modal-open', 'fade', 'animate-open');
    croppieInstance.result({type: 'html', size: 'original', format: 'png', quality: 1, circle: false }).then((html) => {
      const croppedFile = new File([html], 'croppedFile.png', {
        type: 'image/png',
        lastModified: Date.now(),
      });
      const imgUrlResult = URL.createObjectURL(croppedFile);
      imgWrap.style.backgroundImage = `url("${imgUrlResult}")`
